...
g.MouseButton1Click:Connect(function()
	if not isMinimized then
		-- Hide everything except top bar (d, e, and g)
...
pageContainer.Size = UDim2.new(1, 0, 0.75, 0)  -- ~75% of c's height
...
oRename.Size = UDim2.new(0.8, 0, 0.2, 0)
...
guiChangeButton.MouseButton1Click:Connect(function()
	...
	if newText ~= "" and selectedGui then
		local target
		for _, descendant in ipairs(selectedGui:GetDescendants()) do
...
createToolButton.MouseButton1Click:Connect(function()
	...
	if newToolName ~= "" then
		...
		local reflect = 0
		local grip, gripPos
		-- If advanced options are visible, parse them
		if toolOptionsFrame.Visible then
			-- (Optional) tooltip
			local tooltip = tooltipBox.Text or ""
			tool.ToolTip = tooltip
			-- requireshandle and canbedropped
			local req = (string.lower(reqHandleBox.Text) == "true")
			local drop = (string.lower(canDropBox.Text) == "true")
			tool.RequiresHandle = req
			tool.CanBeDropped = drop
			-- grip and grippos (if provided)
			...
			-- handle size, color, material, transparency, reflectance
		...
		handle.Parent = tool
		tool.Parent = bp
		print("new tool created: " .. newToolName)
	else
...
k.MouseButton1Click:Connect(function()
	c.Visible = false
	v.Visible = true
	if updateToolList then updateToolList() end
end)
l.MouseButton1Click:Connect(function()
	c.Visible = false
	ae.Visible = true
	if updateGuiList then updateGuiList() end
end)
m.MouseButton1Click:Connect(function()
	c.Visible = false
	au.Visible = true
